// load study data from the data file
d3.json('../static/data/tabledata.txt', function(study_data){
  global_study_data = study_data;
});
...
function set_visualization_size(){
  ...
  // x_window = 500; //window.innerWidth ;
  // y_window = 500; //window.innerHeight ;
  ...
  // 
  // get screen width 
  screen_width  = Number(d3.select('.content-container').style('width').replace('px',''));
  // get screen height
  screen_height = Number(d3.select('.main-container').style('height').replace('px',''));
  ...
  // container_col_offset = 60;

  // adjust container with border
  // define width and height of clustergram container 
  width_clust_container  = Number(d3.select('.vis-container').style('width').replace('px','')); 
  ...
  d3.select('.vis-container').style('width', width_clust_container+'px')
  d3.select('.vis-container').style('height', height_clust_container+'px')
  ...
  // make the svg width depend on the number of columns
  variable_width = d3.scale.linear().domain([1,10]).range([0.05,1.0]).clamp('true');
  ...
  svg_width  = (width_clust_container  - svg_x_offset - row_label_width) * variable_width(col_nodes.length) ;
